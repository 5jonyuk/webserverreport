...
<head>
    ...
    <title>자료실</title>
    <link rel="stylesheet" href="../css/자료실p.css">
</head>
<body>
    <h1>자료실</h1>
<table>
...
$selrec="select * from file order by file_no desc limit $frow,$line_page";
$info=mysqli_query($conn,$selrec);
while($rowinfo = mysqli_fetch_array($info))
{
    echo "<tr>";
    echo "<td>$rowinfo[file_no]</td>";
    echo "<td><a href='자료실상세보기.php?file_no=$rowinfo[file_no]'>$rowinfo[title]</a></td>";
...
if($cblock>1)
{
   ...
   echo "<a href='자료실.php?page=$prvblock'>◀이전</a>";
}
for($i=$fpage;$i<=$lpage;$i++)
{
	if($i==$page) echo "<b>[$i]</b>";
	else  echo "<a href='자료실.php?page=$i'>[$i]</a>";
}
if($cblock<$totblock)
{
	$nxtblock=($cblock+1)*$block_page;
	echo "<a href='자료실.php?page=$nxtblock'>다음▶</a>";
}
?>
<p><a class="button" href="../html/자료실.html">업로드하러가기</a></p>
